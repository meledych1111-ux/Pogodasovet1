...
// Безопасный хэш (только для проверки входа)
...
async function getDetailedWeatherData(cityName) {
    try {
        ...
        return { success: true, city: name, message: msg, temp: cur.temperature_2m, feels: cur.apparent_temperature };
    } catch (e) { return { success: false, error: e.message }; }
}

// --- СОВЕТЫ ПО ОДЕЖДЕ ---
function getWardrobeAdvice(temp, feels) {
    const t = feels;
    let advice = "🧥 *Рекомендация:*\n";
    if (t >= 25) advice += "Наденьте футболку и шорты ☀️";
    else if (t >= 18) advice += "Подойдет футболка или рубашка 🌤️";
    else if (t >= 10) advice += "Легкая куртка или свитер 🌥️";
    else if (t >= 0) advice += "Демисезонная куртка, шапка 🧥";
    else advice += "Зимний пуховик и теплая шапка ❄️";
    return advice;
}

const dailyPhrases = [
    { e: "Where is the nearest bus stop?", r: "Где ближайшая остановка?" },
    { e: "I'd like a window seat, please.", r: "Я хотел бы место у окна." },
    { e: "Can I have the bill, please?", r: "Можно счет, пожалуйста?" },
    { e: "Nice to meet you!", r: "Приятно познакомиться!" }
];
...
bot.command('start', async (ctx) => {
    ...
    await ctx.reply(`👋 *Привет!* Твой ник: *${cloudName}*\n\nЯ анонимный погодный бот. Выбери город:`, { parse_mode: 'Markdown', reply_markup: cityKeyboard });
});
...
bot.hears('👕 ЧТО НАДЕТЬ?', async (ctx) => {
    ...
    const w = await getDetailedWeatherData(res.city);
    await ctx.reply(getWardrobeAdvice(w.temp, w.feels), { parse_mode: 'Markdown' });
});
...
bot.hears('🎮 ИГРАТЬ В ТЕТРИС', async (ctx) => {
    ...
    // ПЕРЕДАЕМ tg_id ТОЛЬКО ДЛЯ ПРОВЕРКИ ХЭША (не для сохранения)
    const url = `https://pogodasovet1.vercel.app?login=${encodeURIComponent(cloudName)}&hash=${hash}&city=${encodeURIComponent(cityRes.city)}&tg_id=${id}`;
...
bot.hears(/^📍 /, async (ctx) => {
    ...
    await ctx.reply(`✅ Город *${city}* выбран!`, { reply_markup: mainMenuKeyboard, parse_mode: 'Markdown' });
});
