...
// Создаем переменную bot глобально
let bot = null;
...
if (BOT_TOKEN) {
    ...
    console.log('ℹ️ Установите переменную BOT_TOKEN в настройках Vercel');
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (!bot) {
                console.error('❌ Бот не инициализирован! Проверьте BOT_TOKEN');
                return res.status(200).json({ 
                    ok: false, 
                    error: 'Bot not initialized. Check BOT_TOKEN environment variable.'
                });
            }
...
console.log('⚡ Модуль загружен. Бот ' + (bot ? 'готов' : 'НЕ готов') + ' к работе');
