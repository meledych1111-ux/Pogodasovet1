...
bot.hears('🌤️ ПОГОДА СЕЙЧАС', async (ctx) => {
  ...
  if (!pinData) return ctx.reply('Отправь свой ПИН-код (6 цифр) для входа.');
...
bot.hears('🔑 МОЙ ПИН', async (ctx) => {
  const pinData = userPinCache.get(ctx.from.id);
  if (!pinData) return ctx.reply('Нажми /start!');
  await ctx.reply(`Твой ПИН-код: \`${pinData.pin}\``, { parse_mode: 'Markdown' });
});
...
bot.hears(/^\d{6}$/, async (ctx) => {
  ...
  await ctx.reply(`✅ Вход по ПИНу: *${cloudName}*`, { parse_mode: 'Markdown', reply_markup: mainMenuKeyboard });
});
