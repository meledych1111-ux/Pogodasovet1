...
async function getDetailedForecast(cityName, dayOffset = 0) {
  try {
    ...
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,apparent_temperature,precipitation_probability,weather_code&daily=sunrise,sunset,temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=${dayOffset + 1}`;
    ...
    periods.forEach(p => {
      ...
      msg += `${p.e} *${p.n}:* ${t}°C (ощ. ${f}°C)\n   ${getWeatherDescription(data.hourly.weather_code[p.i])}${pr > 5 ? ` | ☔️ ${pr}%` : ''}\n\n`;
    });
...
function getWardrobeAdvice(w) {
  ...
  const { temp, feels_like, city, description, has_rain, has_snow } = w;
  const t = feels_like;
...
bot.command('start', async (ctx) => {
  ...
  const welcome = `👋 *Привет!* Твой анонимный ник: *${name}*\n\nЯ помогу с погодой и английским.\n\n🚀 *Команды:*\n🌤 /weather — Погода сейчас\n📅 /today — На сегодня\n👕 /wardrobe — Что надеть\n💬 /phrase — Английский\n🎮 /tetris — Играть\n\n📍 *Выбери город:*`;
...
bot.hears('🎮 ИГРАТЬ В ТЕТРИС', async (ctx) => {
  ...
  await ctx.reply(`🕹️ *Тетрис*\nНик: *${name}*\n\nНажми кнопку, чтобы играть анонимно:`, {
    parse_mode: 'Markdown',
