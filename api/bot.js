...
// ===================== ДЕТАЛЬНАЯ ПОГОДА НА СЕГОДНЯ (ПО ПЕРИОДАМ) =====================
...
async function getWeatherForecast(cityName, forceRefresh = false) {
  try {
    ...
    const daily = forecastData.daily;
    const sunrise = daily?.sunrise?.[tomorrowDailyIndex]?.substring(11, 16) || '—';
    const sunset = daily?.sunset?.[tomorrowDailyIndex]?.substring(11, 16) || '—';
    ...
    const forecastResult = {
      ...
      temp_min: minTemp !== 100 ? Math.round(minTemp) : null,
      temp_max: maxTemp !== -100 ? Math.round(maxTemp) : null,
      precipitation_tomorrow: daily?.precipitation_sum?.[tomorrowDailyIndex] || 0,
      sunrise: sunrise,
      sunset: sunset,
      day_length: calculateDayLength(sunrise, sunset),
      periods: periodData,
  ...
  try {
    ...
    try {
      ...
      if (ctx && ctx.from) {
        ...
        if (!isInTop && userBestScore > 0) {
          if (userBestScore < 1000) {
            message += `👤 *Ваш результат:* ${userBestScore} очков (нужно 1000+)\n\n`;
          } else {
            message += `👤 *Ваш результат:* ${userBestScore} очков\n\n`;
          }
        }
    ...
    return `❌ Ошибка загрузки топа игроков.`;
  }
}
