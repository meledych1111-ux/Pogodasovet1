 ...
 */
// ==================== ИСПРАВЛЕННАЯ ФУНКЦИЯ СОХРАНЕНИЯ ИГРЫ ====================
export async function saveGameScore(userId, gameType, score, level, lines, username = null, isWin = true) {
  ...
  console.log('📥 Входной userId:', userId);
  ...
  // 🔴 ПРОСТО БЕРЕМ ID КАК ЕСТЬ, ЕСЛИ ОН ЧИСЛОВОЙ!
  let cleanUserId = String(userId).trim();
  
  // Только убираем явные префиксы, если они есть
  if (cleanUserId.startsWith('web_') || cleanUserId.startsWith('test_user_')) {
    cleanUserId = cleanUserId.replace(/^(web_|test_user_)/, '');
  }
  
  // Проверяем что ID числовой
  if (!/^\d+$/.test(cleanUserId)) {
    console.error('❌ Некорректный userId (не числовой):', cleanUserId);
    return { success: false, error: 'ID должен быть числовым' };
  }
  
  console.log('✅ Сохраняем с ID:', cleanUserId);
  ...
  try {
    ...
    try {
      const cityResult = await getUserCity(cleanUserId);
      if (cityResult.success && cityResult.city !== 'Не указан') {
        currentCity = cityResult.city;
      }
    } catch (e) {}
    
    // Сохраняем игру с ТЕМ ЖЕ ID, КОТОРЫЙ ПРИШЕЛ!
    const query = `
    ...
    const result = await client.query(query, [
      cleanUserId,  // 🔴 ТОЛЬКО ЧИСЛОВОЙ ID, БЕЗ ЗАМЕН!
      username || `Игрок_${cleanUserId.slice(-4)}`,
      gameType || 'tetris',
      score || 0,
      level || 1,
      lines || 0,
    ...
    console.log(`✅ Игра сохранена! ID записи: ${savedId}, пользователь: ${cleanUserId}`);
    
    // Удаляем прогресс
    await client.query(
      'DELETE FROM game_progress WHERE user_id = $1 AND game_type = $2',
      [cleanUserId, gameType || 'tetris']
    );
...
export async function getTopPlayers(gameType = 'tetris', limit = 10) {
  ...
  try {
    // 🔴 УБИРАЕМ ФИЛЬТР 1000 ОЧКОВ ВРЕМЕННО!
    const query = `
      SELECT 
        gs.user_id,
      ...
      WHERE gs.game_type = $1 
        AND gs.score > 0  -- 🔴 ЛЮБОЙ ПОЛОЖИТЕЛЬНЫЙ СЧЕТ!
        AND gs.is_win = true
