...
// Вспомогательная функция для выполнения запросов без вложенных коннектов
...
export async function saveOrUpdateUser({ user_id, chat_id, city }) {
    const sql = `
        ...
        ON CONFLICT (user_id) DO UPDATE SET 
            ...
            last_active = NOW()`;
    return await query(sql, [String(user_id), chat_id, city]);
}

export async function getUserProfile(userId) {
    const res = await query('SELECT * FROM users WHERE user_id = $1', [String(userId)]);
...
export async function getTakenLogins() {
    const res = await query('SELECT login FROM game_auth');
    return res.rows.map(r => r.login);
}

export async function saveGameScore(userId, gameType, score, level, lines, username) {
    const profile = await getUserProfile(userId);
    const sql = `
        INSERT INTO game_scores (user_id, username, game_type, score, level, lines, city, created_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`;
    return await query(sql, [String(userId), username || String(userId), gameType, score, level, lines, profile?.city || 'Не указан']);
}
...
export async function getGameStats(userId, gameType = 'tetris') {
    ...
    const res = await query(sql, [String(userId), gameType]);
    const r = res.rows[0];
    return {
...
export const initDb = async () => {
    await query(`
        ...
        CREATE TABLE IF NOT EXISTS game_auth (login VARCHAR(100) PRIMARY KEY, pin_hash VARCHAR(255) NOT NULL, salt VARCHAR(100) NOT NULL, tg_id VARCHAR(100), last_login TIMESTAMP DEFAULT NOW());
        CREATE TABLE IF NOT EXISTS game_scores (id SERIAL PRIMARY KEY, user_id VARCHAR(100), username VARCHAR(100), game_type VARCHAR(50), score INTEGER, level INTEGER, lines INTEGER, city VARCHAR(100), created_at TIMESTAMP DEFAULT NOW());
    `);
};
